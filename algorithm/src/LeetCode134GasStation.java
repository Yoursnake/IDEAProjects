...
public class LeetCode134GasStation {
    public int canCompleteCircuit(int[] gas, int[] cost) {
        int[] diff = new int[gas.length];   // 每个油站的差值

        for (int i = 0; i < gas.length; i++) {
            diff[i] = gas[i] - cost[i];
        }

        int currSum = 0;        // 当前计数总和
        int flag = 0;           // 从这里开始计数
        int oweSum = 0;         // 欠油总和
        for (int i = 0; i < diff.length; i++) {
            currSum += diff[i];
            // 当前计数为负证明不能从当前 flag 开始计数，记下欠油总和，清零当前计数
            if (currSum < 0) {
                oweSum += Math.abs(currSum);
                flag = i + 1;
                currSum = 0;
            }
        }

        if (currSum >= oweSum) return flag; // 如果最终的计数总和能还清欠油则能跑完
        else return -1;
    }
}
