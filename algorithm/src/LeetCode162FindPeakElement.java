...
public class LeetCode162FindPeakElement {
    ...
    public int findPeakElement(int[] nums) {
        ...
        return findPeakElement(nums, 1, len - 2);
    }

    private int findPeakElement(int[] nums, int begin, int end) {
        int mid = (begin + end) / 2;
        if (nums[mid] > nums[mid - 1] && nums[mid] > nums[mid + 1]) return mid;
        else if (nums[mid - 1] < nums[mid] && nums[mid] < nums[mid + 1]) return findPeakElement(nums, mid + 1, end);
        else return findPeakElement(nums, begin, mid);
    }
}
