...
public class LeetCode165CompareVersionNumbers {
    public int compareVersion(String version1, String version2) {
        ...
        int maxLen = Math.max(v1Split.length, v2Split.length);
        int[] v1Val = new int[maxLen];
        int[] v2Val = new int[maxLen];

        for (int i = 0; i < v1Split.length; i++) v1Val[i] = Integer.parseInt(v1Split[i]);
        for (int i = 0; i < v2Split.length; i++) v2Val[i] = Integer.parseInt(v2Split[i]);

        for (int i = 0; i < maxLen; i++) {
            if (v1Val[i] < v2Val[i]) return -1;
            if (v1Val[i] > v2Val[i]) return 1;
        }
